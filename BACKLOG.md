...
size. The workspace is still at `3.1.0`: **3.2.0 is unreleased and this cycle is open.**
...
The **six extension rows (`S17`, `S19`–`S23`) are now planned**, carved into five plans —
[`plans/000-index-3-2-extensions.md`](plans/000-index-3-2-extensions.md) — with build order,
shared-file edges, and set-level done criteria. ⚠️ They are **planned, not shipped**: they stay in
the Live table until verified present in source, the same standard `S1`–`S16` were held to.
...
|----|---------|----------|------------|--------|-----------|----------|
| `S17` | **Decide `MetricsMiddleware`'s position relative to `TracingMiddleware`** — it is a decision, not a doc fix | 🟡 should | S | `✅ planned → plans/041` | Filed by [`plans/035`](plans/035-http-edge-hardening.md) §D-order-bugs and **still unresolved**. `MetricsMiddleware` executes *outside* `TracingMiddleware` (`app.py:586-592`); Plan 035 corrected the two comments that claimed the opposite and stopped there, which documented the behaviour without deciding it. Consequence today: metrics are recorded with **no active span context**, so no OTel exemplars and no trace↔metric correlation. Either that is intentional and the reason belongs in Answered decisions, or it is a one-line move | `varco_fastapi/varco_fastapi/app.py:586-592`; `varco_fastapi/varco_fastapi/middleware/__init__.py:14-36` |
| `S19` | **Inbound webhook signature verification** — the missing half of the outbound signer | 🟡 should | S | `✅ planned → plans/038` | varco signs **outbound** webhooks (`StandardWebhooksSigner`, `Rfc9421Signer`) and verifies **nothing inbound** — verified absent across all ten packages. Every varco app that *receives* a Stripe/GitHub/Svix webhook hand-rolls HMAC compare, timestamp tolerance, and a replay window today, which is exactly the code people get wrong (non-constant-time compare, no replay bound). It is a mirror of shipped code in `varco_core.webhook`, with no new dependency and no new concept | scout: verified absence; brief 011 §C — Standard Webhooks is the de facto spec and varco already implements its signing half |
| `S22` | **JWKS background refresh task** for `TrustedIssuerRegistry` | 🟡 should | S | `✅ planned → plans/041` | CLAUDE.md already documents this hole with a ⚠️: *"There is no background refresher task — a registry that never receives a `verify()` call never refreshes on its own regardless of these knobs."* The `min_refresh_interval`/`ttl_seconds` knobs therefore under-deliver on their own documentation. It was deferred for needing lifespan start/stop wiring — machinery `varco_fastapi` has shipped since (`MigrationLifecycle`, `TenancyLifecycle`, `ReliabilityLifecycle`, `SecurityPostureLifecycle` are all precedents) | CLAUDE.md's own ⚠️ in the Authority/JWT section; `technical_docs/features/jwt-claim-transformer.md:292-314` |
| `S23` | **Conformance-guard recovery** — restore the lost `strict=True` xfail findings and fix CLAUDE.md's dangling pointer | 🟡 should | S | `✅ planned → plans/042` | CLAUDE.md's Test Conventions instructs that a conformance failure revealing a backend ABC violation becomes a `strict=True` xfail plus a BACKLOG row, and points at *"BACKLOG.md's 'Known issues found while implementing Plan 012' table"*. **That table no longer exists** (trimmed in an earlier pass) and there are **zero** `strict=True` xfails carrying a `BUG:` reason anywhere in the test tree. So either those bugs were fixed and the convention was never applied, or the findings were lost with the table — and today the repo documents a guard it does not have. Re-verify the named findings (`RedisCache`/`MemcachedCache` truncating a sub-second `ttl` to `int()`; `KafkaDLQ`/`NatsDLQ.delete_where()` never reaching the ABC's no-predicate `ValueError`), fix or guard each, and repoint CLAUDE.md | CLAUDE.md Test Conventions vs. `testkit/varco_conformance/__init__.py:26`; verified absence of any `BUG:` xfail |
| `S20` | **Retention & purge automation** — a `RetentionPolicy` registry materialized onto the shipped cron→Job path | 🟡 should | S–M | `✅ shipped → plans/039` | Every subsystem already exposes its own cleanup verb — DLQ retention, audit retention, `IdempotencyStore.delete_expired()`, `TokenRevocationStore.delete_expired()` — and **nothing schedules any of them**. ⚠️ **Correction (Plan 039's own §D-S20-exists audit):** this row's original framing named "outbox pruning" as an existing cleanup verb — it **does not exist** (`OutboxRepository` exposes only `save`/`get_pending`/`delete(entry_id)`, `service/outbox.py:62-64`) and deliberately stays out of scope, since deleting an unpublished outbox row by age is event loss by construction. The job store is *already* swept by `JobPoller(retention_sweep=True)` (`varco_fastapi/varco_fastapi/job/poller.py:75-88`) — `JobRetentionTarget` exists for a uniform surface, not because nothing covered it. An operator's only option was to hand-write a job per subsystem, which is precisely the wiring varco exists to own. `varco_core.schedule` (Plan 032 / D6) shipped the cron→`Job` materializer this needed, so this was composition of two shipped subsystems plus three small driver gaps Plan 039 closed in that subsystem itself (`materialize()` had no caller; a materialized `Job` had no `task_payload`; `JobRunner.recover()` ignored `Job.run_at`) | scout: verified absence of any scheduler over the existing verbs; `intuition` on the framing; Plan 039's own §D-S20-exists table, every row cited against source |
...
| `S21` | **Unified redaction seam** (`varco_core.redaction`) — one `Redactor` behind spans, logs, audit payloads, and `error_params()` | 🟡 should | M | `✅ planned → plans/040` | Redaction exists in **exactly one place** — `varco_core/observability/params.py` — and nothing else reuses it. The three other surfaces that carry user data outward have no shared defence: the logging middleware, the audit trail's before/after JSON, and `error_params()`, which **CLAUDE.md itself flags** as *"a new exfiltration surface"* whose overrides must "apply the same scrutiny, never `vars(exc)`" — a rule with no mechanism behind it. ⚠️ This is the one extension row that adds a **public seam** rather than plugging a hole; it earns that by being the only one with a live data-leak shape | scout: verified single-site redaction; CLAUDE.md's own ⚠️ on `error_params()`; brief 011 §B (secret/PII handling) |
...
`S1`–`S16`, all five plans, **verified present in source 2026-09-07** (not merely marked done).
Detail lives in the plan files, `CHANGELOG.md`, and `technical_docs/features/`.
