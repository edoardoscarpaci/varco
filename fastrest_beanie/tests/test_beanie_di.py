"""
Tests for the providify DI integration module (fastrest_beanie.di).
...
from fastrest_beanie.di import (
    BeanieModule,
    BeanieSettings,
    _make_repo_provider,
    bind_repositories,
...
async def test_beanie_module_repository_provider_returns_repository_provider() -> None:
    ...
    with (
        patch(
            "fastrest_beanie.di.BeanieRepositoryProvider", autospec=True
        ) as mock_provider_cls,
        patch("beanie.init_beanie", new_callable=AsyncMock),
        patch("fastrest_beanie.provider.BeanieDocRegistry") as mock_registry,
    ):
...
async def test_beanie_module_repository_provider_calls_init() -> None:
    ...
    with patch("fastrest_beanie.di.BeanieRepositoryProvider") as mock_cls:
