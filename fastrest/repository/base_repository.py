...
from typing import TYPE_CHECKING, AsyncGenerator, Generic, List, Type, Optional, Any
...
from fastrest.exception.repository import (
    FieldNotFound,
...
class AutoRegisterRepositoryMeta(ABCMeta):
    ...
    def __new__(cls, name: str, bases: tuple, attrs: dict) -> type:
        ...
        if database_model_class is None:
            raise RepositoryClassCreationFailed(
                f"Failed to create {new_cls.__name__} missing 'database_model_class' ClassVar make sure to set when subclassing Repository",
                repository_cls=new_cls,
            )
        ...
        if entity_class is None:
            warnings.warn(
                f"Missing entity_class skipping autoregistration for {name}",
                RegistrationFailedWarning,
            )
            return new_cls
...
class Repository(
    Generic[TEntity, TDatabaseModel, TCreateDTO, TReadDTO, TUpdateDTO],
    metaclass=AutoRegisterRepositoryMeta,
):
    """
    ...
    def __init__(
        self,
        *args,
        model_assembler: Optional[
            ModelAssembler[TEntity, TDatabaseModel, TCreateDTO, TReadDTO]
        ] = None,
        **kwargs,
    ):
        self.model_assembler = model_assembler or ModelAssemblerRegistry.instance().get(
            key=self.entity_class
        )
    ...
    @abstractmethod
    async def create(
        self, entity: TCreateDTO, *args, db_session: Optional[Any] = None, **kwargs
    ) -> TReadDTO:
        """
    ...
    async def _get_model(
        self, entity_id: str, *args, db_session: Optional[Any] = None, **kwargs
    ) -> TDatabaseModel:
        """
    ...
    @abstractmethod
    async def _save(
        self, model: TDatabaseModel, *args, db_session: Optional[Any] = None, **kwargs
    ) -> TDatabaseModel:
        """
    ...
    @abstractmethod
    async def list(
        self, pagination: Pagination, *args, db_session: Optional[Any] = None, **kwargs
    ) -> List[TReadDTO]:
        """
    ...
    @abstractmethod
    async def stream(
        self, pagination: Filter, *args, db_session: Optional[Any] = None, **kwargs
    ) -> AsyncGenerator[TReadDTO, None]:
        """
    ...
    async def exists(
        ...
        try:
            model = await self._get_model(
                entity_id=entity_id, *args, db_session=db_session, **kwargs
            )
    ...
    async def get(
        ...
        model_db = await self._get_model(
            *args, entity_id=entity_id, db_session=db_session, **kwargs
        )
    ...
    async def update(
        ...
        **kwargs,
        ...
        saved_model = await self._save(
            *args, model=model, db_session=db_session, **kwargs
        )
    ...
    def _set_attribute(
        ...
        if not hasattr(model, field_name):
            raise FieldNotFound(
                field=field_name, table=self.database_model_class.__tablename__
            )
