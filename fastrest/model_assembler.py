...
class AutoRegisterModelAssemblerMeta(ABCMeta):
    def __new__(cls, name, bases, attrs):
        ...
        if entity_cls is None:
            warnings.warn(
                f"Missing entity_cls skipping autoregistration for {name}",
...
class ModelAssembler(
    Generic[TEntity, TDatabaseModel, TCreateDTO, TReadDTO],
    metaclass=AutoRegisterModelAssemblerMeta,
):
    ...
    def to_read_dto_from_entity(self, entity: TEntity) -> TReadDTO:
        ...
        for field_name, field_info in self.read_entity_class.model_fields.items():
            if hasattr(entity, field_name):
                ...
                raise FieldNotFoundInEntity(
                    field_name=field_name, entity_class=type(entity)
                )
