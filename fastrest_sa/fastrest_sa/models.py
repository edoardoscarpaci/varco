...
DESIGN: moved from ``fastrest_core.models.database_model``
  Rationale: ``BaseDatabaseModel`` directly subclasses SQLAlchemy's
  ``DeclarativeBase``, making it SA-specific.  Placing it in
  ``fastrest_core`` would force a SQLAlchemy import in a package that
  is otherwise ORM-agnostic.  It now lives in ``fastrest_sa`` alongside
